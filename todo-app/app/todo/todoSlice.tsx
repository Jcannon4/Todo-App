...
const todoSlice = createSlice({
    ...
    reducers: {
        ...
        toggleComplete: (state, action: PayloadAction<{ id: string, isComplete: boolean }>) => {
            ...
            if (curItem.isComplete) {
                ...
                const startIndex: number = state.completed.findIndex(item => item.id === curItem.id);
                ...
                const isolatedArray: TodoItemProps[] = state.completed.splice(startIndex, 1); // magic number 1 indicates we remove 1 item
                ...
                const startIndex: number = state.incomplete.findIndex(item => item.id === curItem.id);
                // splice() item from array (memory complexity increase, since copy array is formed)
                // Also catch the removed item in an array of type TodoItemProps
                const isolatedArray: TodoItemProps[] = state.incomplete.splice(startIndex, 1); // magic number 1 indicates we remove 1 item
                // toggle the isCompete status
                isolatedArray[0].isComplete = !isolatedArray[0].isComplete;
        ...
        deleteTodo: (state, action: PayloadAction<{ id: string, isComplete: boolean }>) => {
            console.log(" FIRING DELETE FROM REDUCER, deleting item with id: \n" + action.payload.id + ' ' + action.payload.isComplete);
            ...
            if (action.payload.isComplete) {
                ...
                state.incomplete = state.incomplete.filter(item => item.id !== action.payload.id)
            }
        }
